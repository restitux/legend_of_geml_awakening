...
void room_draw_tile(uint32_t tile, uint32_t x, uint32_t y, const uint8_t *tileset) {
...
void room_draw_room(uint32_t room_x, uint32_t room_y, const struct Tilemap *map) {
    ...
    for (int x = room_start_x; x < room_start_x + 20; x++) {
        for (int y = room_start_y; y < room_start_y + 20; y++) {
            ...
            room_draw_tile(map->map[tile_index], x - room_start_x, y - room_start_y, map->tileset);
        }
    }
}