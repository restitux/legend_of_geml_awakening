...
void sprite_draw_sprite_frame(const struct SpriteFrame *sprite, const struct ScreenCoordinate *loc) {
...
void sprite_advance_animation(struct CharacterSprite *sprite) {
    ...
    sprite->current_frame %= (sprite->sheet_height / sprite->frame_size) * sprite->frames_per_animation;
}

void sprite_draw_character(const struct CharacterSprite *sprite, const struct ScreenCoordinate *loc, enum Direction dir) {
